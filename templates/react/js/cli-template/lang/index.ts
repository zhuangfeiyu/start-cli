...
const dayjsLocaleMap = { [LangEnum.zhCN]: 'zh-cn', [LangEnum.en]: 'en' }
const antdLocaleMap = { [LangEnum.zhCN]: zhCNLocale, [LangEnum.en]: enLocale }

export function setLocale(locale) {
  const dayjsLocale = dayjsLocaleMap[locale] ?? 'zh-cn'
  dayjs.locale(dayjsLocale)
  i18n.changeLanguage(locale)
}

export function getAntdLocale(locale) {
  return antdLocaleMap[locale] ?? zhCNLocale
}
